...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 20:30:35 2020
...
def MHsampler2(w0,b2est,shapes_prior,rates_prior,s1,s2,std,P,binsize,seconds,U,it,tau):
    ...
    old_log_post = particle_filter(w0,b2est,tau,s1,s2,std,P,binsize,seconds,theta_prior)
    for i in range(1,it):
        if (i % U == 0):
        ...
        new_log_post = particle_filter(w0,b2est,tau,s1,s2,std,P,binsize,seconds,theta_next)
...
w0ests = []
for i in range(5000):
    s1temp,s2temp,ttemp,Wtemp = generative(Ap, Am, tau, tau, b1, b2, w0, std, seconds, binsize)
    b1est = infer_b1(s1temp)
    b2est = infer_b2_w0(s1temp, s2temp, 1e-10)[0]
    w0est = infer_b2_w0(s1temp[:5000], s2temp[:5000], 1e-10)[1]
    W5ms.append(Wtemp)
    b1ests.append(b1est)
    b2ests.append(b2est)
    w0ests.append(w0est)

np.save('Trajectories2ms_bm2.75',W5ms)
np.save('B1estimates2ms_bm2.75',b1ests)
np.save('B2estimates2ms_bm2.75',b2ests)
np.save('w0estimates2ms_bm2.75',w0ests)