...
w01 = np.load('w0estimates1ms_bm3.5.npy')
b11 = np.load('B1estimates1ms_bm3.5.npy')
b21 = np.load('B2estimates1ms_bm3.5.npy')

ciw01 = stats.norm.interval(0.95,np.mean(w01),np.sqrt(np.var(w01)))
cib11 = stats.norm.interval(0.95,np.mean(b11),np.sqrt(np.var(b11)))
cib21 = stats.norm.interval(0.95,np.mean(b21),np.sqrt(np.var(b21)))

plt.figure()
plt.title('W0 estimation, 1ms gridsize')
sns.set_style('darkgrid')
sns.distplot(w01, norm_hist = True)
plt.axvline(1,color='r',linestyle='--',label='True Value')
plt.axvline(ciw01[0],color='g',linestyle='--')
plt.axvline(ciw01[1],color='g',linestyle='--',label='95% CI')
plt.legend()
plt.show()

plt.figure()
plt.title('$b_1$ estimation, 1ms gridsize')
sns.set_style('darkgrid')
sns.distplot(b11, norm_hist = True)
plt.axvline(-3.5,color='r',linestyle='--',label='True Value')
plt.axvline(cib11[0],color='g',linestyle='--')
plt.axvline(cib11[1],color='g',linestyle='--',label='95% CI')
plt.legend()
plt.show()

plt.figure()
plt.title('$b_2$ estimation, 1ms gridsize')
sns.set_style('darkgrid')
sns.distplot(b21, norm_hist = True)
plt.axvline(-3.5,color='r',linestyle='--',label='True Value')
plt.axvline(cib21[0],color='g',linestyle='--')
